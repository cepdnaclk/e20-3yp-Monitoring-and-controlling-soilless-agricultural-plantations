...
it('triggers multiple alerts and control commands for multiple mismatches', async () => {
  firestore.onSnapshot.mockImplementation((ref, callback) => {
    if (ref.path?.includes('control_settings')) {
      callback({
        exists: () => true,
  ...
  await waitFor(() => {
    expect(utils.sendControlCommand).toHaveBeenCalledTimes(5);
  });
});

it('does not trigger alerts for values within safe margin', async () => {
  firestore.onSnapshot.mockImplementation((ref, callback) => {
    if (ref.path?.includes('control_settings')) {
      callback({
        exists: () => true,
...
it('removes alert from UI when dismissed', async () => {
  ...
  const dismissButtons = getAllByText('Dismiss');
  fireEvent.press(dismissButtons[0]); // 🔧 dismiss the first alert only
